...
public class HotelAnalysis {
	...
	public static int getTotalRates(TreeMap<String, Reservation> reservationList, Date start, Date end, String hotelType, String roomType) {
		ArrayList<Reservation> res = AnalyseHotels(reservationList, start, end);
		int total = 0;
		for(int i=0; i<res.size(); i++) {
			if(hotelType.equals(res.get(i).getRoom().getRoomType().getHotelType()) && roomType.equals(res.get(i).getRoom().getRoomType().getType())) {
				total = (int) Double.parseDouble(res.get(i).getPrice()) + total;
			}
